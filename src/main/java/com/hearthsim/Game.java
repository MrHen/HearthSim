...
public class Game {
	...
	public GameResult runGame() throws HSException {
		...
		record.put(0, PlayerSide.CURRENT_PLAYER, boardModel_.deepCopy(), null);
		record.put(0, PlayerSide.CURRENT_PLAYER, boardModel_.flipPlayers().deepCopy(), null);
	...
	private GameResult playTurn(int turnCount, GameRecord record, ArtificialPlayer ai) throws HSException {
		...
		List<HearthActionBoardPair> allMoves = ai.playTurn(turnCount, boardModel_);
		if(allMoves.size() > 0) {
	...
	public BoardModel beginTurn(int turn, BoardModel board) throws HSException {
		...
		if(newCard == null) {
			...
			toRet.data_.getCurrentPlayerHero().setHealth(
					(byte)(toRet.data_.getCurrentPlayerHero().getHealth() - fatigueDamage));
		} else {
