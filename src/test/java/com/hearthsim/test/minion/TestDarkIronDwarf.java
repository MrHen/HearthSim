...
public class TestDarkIronDwarf {
	...
	@Test
	public void testAddedToBattlefield() throws HSException {
	...
	@Test
	public void testBuffChildrenAdded() throws HSException {
		Minion target = board.data_.getCharacter(PlayerSide.CURRENT_PLAYER, 2);
		...
		assertEquals(board.getChildren().get(0).data_.getCurrentPlayer().getMinions().get(0)
				.getExtraAttackUntilTurnEnd(), 2);
		assertEquals(board.getChildren().get(1).data_.getCurrentPlayer().getMinions().get(1)
				.getExtraAttackUntilTurnEnd(), 2);
		assertEquals(board.getChildren().get(2).data_.getWaitingPlayer().getMinions().get(0)
				.getExtraAttackUntilTurnEnd(), 2);
		assertEquals(board.getChildren().get(3).data_.getWaitingPlayer().getMinions().get(1)
				.getExtraAttackUntilTurnEnd(), 2);

		// TODO move this section of the test into Minion as an ExtraAttackUntilTurnEnd test
